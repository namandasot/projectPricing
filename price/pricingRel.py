...
class PricingScore:
	def __init__(self): 
		...
		self.websiteLeadFactor = 1.0/3
		self.selfPriceFactor = 1.1
		self.cherryPickPreFactor = 1.45
		self.cherryPickPostFactor = 1.6
	...
	def cost_factor(self,relList):
		...
		for proj in relList:
			relProj.append(proj)
		# print relProj
		relProj = [int(a) for a in relProj]
		relProjStr = str(relProj).replace("[",'(').replace("]",")")
		prevMonth =  currDate - datetime.timedelta(days=90)
		query = "Select project_id,sum(cost),sum(lead_ad) from cost_output_july where date >= \""+ str(prevMonth) + "\" and project_id in "+relProjStr+" group by project_id"
		startDate = "2016-04-01"
		endDate = "2016-06-30"
		query = "select project_id,sum(cost),sum(lead_ad),sum(ProjectOpenCount),sum(ProjectViewCount) from cost_output_july where project_id in "+relProjStr+" and date between \""+ str(startDate) + "\" and \"" +str(endDate)+"\" and device_name=\"computers\" group by project_id"
...
# 		query = "Select Project_No from project_enquiry_requests Where Created_Dt >= \"" + str(prevMonth) +"\""
	...
	def pricingLeads(self,budget,locations,BHK,possesion):
		returnDict = {}
...
# 		print budget,locations,BHK,possesion
		try:
			# for i in propList:
			locationArr = locations
			locations = str(locations).replace("[",'(').replace("]",")")
			query = "Select Project_No,Project_City,Project_Area,Project_Config_No,No_Of_Bedroom,Minimum_Price,Possession,Maximum_Price from all_project_info where Project_Area in "+ locations
	...
	def getPrices(self,propList):
		for proj in propList:
			x = propList[proj]
			propList[proj] = [max(self.minPricingLead, int(self.websiteLeadFactor*x)) , max(self.minPricingLead,int(x*self.selfPriceFactor)) ,max(self.minPricingLead,int(self.cherryPickPreFactor * x )), max(self.minPricingLead,int(self.cherryPickPostFactor * x ))]

		return propList

	def pricingRelScore(self,budget,location,BHK,possesion,propList):
		filterArray = [1]*(len(propList))
		pricingRelScoreArr = self.getBudgetScore(budget,propList[:,5],filterArray)
		pricingRelScoreArr += self.getBHKScore(BHK,propList[:,4],filterArray) 
		pricingRelScoreArr += self.getPossessionScore(possesion,propList[:,6],filterArray)
		self.getLocationScore(location,propList[:,2],filterArray)
		pricingRelScoreArr =  pricingRelScoreArr/3.0		

		projectConfigNoList = propList[:,3]
		d = {}
		for i,projectConfigNo in enumerate(projectConfigNoList):
			# print projectConfigNo
			if(filterArray[i]==1):
				if(pricingRelScoreArr[i] > 6.5):
					d[projectConfigNo] = (pricingRelScoreArr[i] )/10.0
		return d

	def getPossessionScore (self,possesion,possesionList,filterArray):
		pList = []
		for i in possesionList:
			poss = i-datetime.date.today()
			poss = poss.days
			if(poss < 0):
				poss = 0
			pList.append(poss)
	...
	def getBudgetScore(self,searchBudget,propList,filterArray):
		budgetScore = 0
		budgetScoreList = []
		for i,propPrice in enumerate(propList):
			if(propPrice > searchBudget*1.25 or propPrice < searchBudget*0.7):
				filterArray[i] = 0
			...
			if(searchBudget >= propPrice):
				...
				budgetScore = self.getLineValueAll(searchBudget,10,searchBudget*1.4,2,propPrice)
	...
	def getLocationScore(self,searchLocations,propLocationList,filterArray):
		locationScore = 0
		locationScoreList = []
		...
		for i,propLocation in enumerate(propLocationList) : 
			locationScore = 10
# 			
# 			if propLocation in searchLocations:
# 				locationScore =10
# 			else:
...
# 				locationScore = 0
	...
	def getBHKScore(self,searchBHK,propList,filterArray):
		bhkScore = 0 
		bhkScoreList = []
		for i,propBHK in enumerate(propList) : 
			# handling LAND
			if propBHK == "":
				propBHK = -1
			propBHK = float(propBHK)
			if (searchBHK == -1):
				if(propBHK <= 0 ):
					bhkScore = 10
				else:
	...
	def price1(self,relList,propList):
...
# 		locationListStr = str(locationList).replace("[",'(').replace("]",")")
		...
		"""		
		cur=self.db.cursor()
		query = "Select Project_No, Project_Config_No,Project_City, Minimum_Price,Maximum_Price from all_project_info where Project_Area in " + locationList
		# print query
		cur.execute(query)
		projectCityMap = []
		for row in cur.fetchall():
		...
		for projectConfigNo in relList:
			for projectDetails in propList:
				if projectConfigNo == projectDetails[3]:
					projectNo = projectDetails[0]
					city = projectDetails[1]
					priceProject = (float(projectDetails[5])+float(projectDetails[7]))/2
	...
	def quality_factor(self,priceDict):
		...
		query = "Select Project_No from project_enquiry_requests Where Created_Dt >= \"" + str(prevMonth) +"\" and Project_No in " + relProjStr
		# print query
		cur.execute(query)
...
if __name__ == '__main__':
	...
	print p.pricingLeads(6460000,[321],1,90)
