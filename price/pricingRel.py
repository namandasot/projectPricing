	...
	def price1(self,lst):
		...
		for key in lst:
			for rows in projectCityMap:
				Price_project = (float(rows[2])+float(rows[3]))/2
				if rows[0]==key:
					city=rows[1]
					for row in cplPricing:
						...
						if city == row[0]:
							...
							Price_project = Price_project/100000 
						

							if(Price_project < min_price_range):
								cpl_amount_basic = min_cpl
							elif(Price_project<max_price_range):
								cpl_amount_basic= (max_cpl-min_cpl)/(max_price_range-min_price_range)*Price_project + min_cpl - (max_cpl-min_cpl)/(max_price_range-min_price_range)*min_price_range
							elif(Price_project<max_price_range_second):
								cpl_amount_basic=(max_cpl_second-max_cpl)/(max_price_range_second-max_price_range)*Price_project + max_cpl - (max_cpl_second-max_cpl)/(max_price_range_second-max_price_range)*max_price_range
							elif(Price_project > max_price_range_second):
...
if __name__ == '__main__':
	...
	print p.pricingLeads(6460000,['Malad'],0,recoPropAttrList)
